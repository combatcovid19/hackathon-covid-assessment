...
public class AssessmentService {
	...
	public AssessmentScoreAndProviders saveAssessmentAnswers(AssessmentAnswerInfo assessmentAnswerInfo) {
		...
		try {
			...
			if (!CollectionUtils.isEmpty(assessmentAnswerEntities)) {
				...
				if (assessmentScore >= 60) {
					...
					GeocodingResult geocodingResult = retrieveGeoLocationFromZipCode(

							String.valueOf(
									assessmentAnswerInfo.getProfileAndBiomedicalAuthorityInfo().getProfileZipCode()));
...
//			if (!CollectionUtils.isEmpty(nearByProvidersToAssesse)) {
				...
				assessmentScoreAndProviders = assessmentTransformer.convertProviderEntityToModel(nearByProvidersToAssesse, assessmentScore);
